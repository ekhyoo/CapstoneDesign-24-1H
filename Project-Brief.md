...
| (8-4) 과제: 과제의 주요 기능 | [회원 도메인 기능 리스트] - 사용자들의 로그인과 회원가입에 관련된 기능을 제공합니다.
<br><br>
[일기장 도메인 기능 리스트] - 일기장을 등록/비활성화/삭제/회수하는 기능을 제공합니다. 두 명의 사용자가 연결되어 하나의 일기장을 가지게 됩니다.<br>
- 일기장 비활성화<br>
    일기장을 비활성화 하는 경우 일기 열람만 가능하며 더 이상의 일기 작성은 불가능 합니다. 두 명의 사용자 중 한 명이라도 일기장 비활성화를 할 경우 해당 일기장은 비활성화 됩니다. <br>
- 일기장 삭제 <br>
    일기장 정보를 더 이상 조회할 수 없습니다. 모든 데이터가 삭제됩니다.   <br>
- 일기장 회수    <br>
    일정 시간이 지났음에도 상대방에서 답장을 받지 못하는 경우 일기 회수 요청을 통해 일기장을 반환 받아서 작성할 수 있습니다.<br>
   <br><br>
[일기 도메인 기능 리스트] - 일기를 작성/수정/전달하는 기능을 제공합니다. 하나의 일기장에 여러 장의 일기(entry)가 속하게 됩니다.<br>
- 일기 작성/수정<br>
    일기를 작성하고 저장할 수 있습니다. 작성한 후 일기를 바로 전달하지 않고 추후에 작성했던 일기를 불러와 추가로 작성하는 것도 가능합니다. <br>
- 일기 전달<br>
    일기 작성 후 상대방에게 일기 전달이 가능합니다.<br>
<br><br>
[매칭 도메인 기능 리스트] - 지인과 매칭 혹은 익명의 친구와 매칭하는 기능을 제공합니다. 매칭이 된 후에는 매칭 히스토리 테이블에 업로드하여 중복 매칭을 방지합니다.<br> 
- 지인 매칭 기능<br>
    일기장 생성 후 생성되는 고유한 코드를 이용하여 지인과 일기장 생성을 완료할 수 있습니다.<br>
- 랜덤 매칭 기능<br>
    익명의 친구와 매칭의 경우 관심사 3개 및 매칭 요청일을 기반으로 한 알고리즘을 통해 랜덤 매칭을 합니다.  <br>
    <br><br>
[감정 분석 레포트 기능 리스트]
SKT kobert 모델을 파인 튜닝하여 일기 내용을 행복, 불안, 분노, 당황, 슬픔, 혐오 6개의 감정으로 분류하도록 합니다. 일기 전달 시 작성한 일기를 바탕으로 감정을 분석하고 결과로 레포트를 제공합니다. |
...
- backend: Spring Boot, Gradle<br>
- CI/CD: Github Actions<br>
- container: Docker<br>
2. 감정 분석을 위한 ai 모델으로 skt koBERT 사용<br>
- 감정 6 개 행복, 당황, 슬픔, 분노, 불안, 혐오를 구분할 수 있도록 파인튜닝 진행<br>
- 파인 튜닝 후 실제 일기와 비슷한 값을 넣어 테스트한 결과 원하는 분류대로 결과를 도출하는 것을 확인<br>
3. 스프링 부트와 AWS EC2를 이용한 API 서버와 CloudFront+S3를 이용한 리액트 웹 서버 구축 <br>
4. MariaDB RDBMS 와 AWS RDS 를 이용한 데이터베이스 구축<br> |
| (8-7) 과제: 기대 효과 및 의의 | |
| (9) 데모 내용 |
1. 로그인<br>
   사용자1, 사용자2의 두 개의 계정으로 각각 로그인한다.<br>
2. 지인 매칭을 통한 일기장 생성<br>
   2-1. 사용자1 계정에서 지인 매칭을 통해 일기장을 생성한 후 사용자2에게 초대코드를 전달한다.<br>
   2-2. 사용자2가 전달받은 초대코드를 입력한다.<br>
   2-3. 일기장 생성완료<br>
3. 일기 작성<br>
   3-1. 사용자1이 만들어진 일기장을 확인한다.<br>
   3-2. 새로운 일기 내용을 작성 후 저장한다.<br>
4. 일기 전달<br>
   4-1. 사용자1이 일기를 전달한다.<br>
   4-2. 전달 완료된 일기장은 사용자2가 확인할 수 있다.<br>
5. 감정 분석 레포트 확인<br>
   5-1. 사용자1은 일기를 전달 한 뒤 자신의 일기 내용을 분석한 감정 분석 레포트를 확인할 수 있다.  |
| (10) 기타 |  |
