...
| (8-1) 과제: 문제의 정의 | 최근 물가 상승으로 인해 위축된 소비 심리를 자극하지 않으면서 이윤을 취하려는 기업이 많아지며 제품의 가격은 그대로 유지하는 대신 제품의 용량을 줄이는 '슈링크플레이션'과 가격을 인상하는 대신 제품의 품질을 낮추는 '스킴플레이션'이 빈번히 발생하고 있습니다. 또한 설문 조사 결과, 정보의 부재 때문에 이런 현상에 적절하게 대응하지 못하는 소비자들이 다수 있음을 확인했습니다. 그럼에도 식료품의 용량이나 원재료 정보 등을 트래킹 및 시각화하여 변화 정도를 보기 쉽게 알려주고, 비합리적인 가격 인상이나 원재료 변경이 생긴 제품을 대신할 대체품을 추천해주는 서비스가 없습니다. 따라서 저희는 물가 상승으로 인해 변동 사항이 많은 식료품 업계에서 현명한 소비를 하고자 하는 이들에게, 슈링크/스킴플레이션을 감지하고 비합리적인 상품에 대해서는 대체제를 찾는 소비를 돕는 서비스를 제공하고자 합니다. |
| (8-2) 과제: 기존연구와의 비교 | 기존에도 소비자들에게 식료품의 용량과 원재료 정보를 알려주는 웹 서비스는 존재했습니다. 대표적인 것이 공공기관인 한국 소비자원에서 운영하는 '소비자24', '참가격' 과 민간에서 운영하는 '다나와', '에누리' 사이트입니다. 가장 먼저 '소비자24' 사이트는, 상품명 검색을 통해, 구매를 원하는 식료품의 중량과 원재료 정보를 한 페이지에서 확인할 수 있습니다. 더 나아가 '참가격' 사이트에서는 마찬가지로 상품명 검색을 통해, 식료품의 현재 용량과, 1주전,	1개월전,	6개월전,	1년전 가격을 한 페이지에서 확인할 수 있습니다. 한편 민간 사이트인 '다나와'와 '에누리' 는 1개월전, 3개월전, 6개월전, 1년전 최저가 그래프와 제품의 용량을 한 페이지에서 확인할 수 있습니다. 그러나 위에 언급된 네 가지 서비스 중 어느 것도 상품의 과거 용량 변동 정보와 원재료 구성 변동 정보를 확인할 수 없었습니다. 아울러 네 서비스 모두 식료품을 검색하는 절차가 상품명 검색 하나로 제한되어 있었으며, 변동 사항이 생겼을 때 이를 소비자에게 직접적인 알림으로 전달하는 기능과 소비자의 구매 판단에 직접적인 도움을 주는 기능은 제공하고 있지 않았습니다.
|
| (8-3) 과제: 제안 내용 | 1) 소비자들이 슈링크/스킴플레이션이 발생한 사실을 늦지 않게 알 수 있어야 합니다. 이에 저희는 알림 기능을 생각하여 슈링크/스킴플레이션이 발생한 식료품에 대해 사용자가 바로 알림을 받아볼 수 있게 하였고, 홈 화면에서 슈링크/스킴 플레이션이 발생한 항목들을 소비자들이 확인할 수 있게끔 하였습니다. <br>2) 소비자들이 슈링크/스킴플레이션이 발생한 식료품에 대해 그것이 일어나기 전의 용량, 원재료에 대한 정보를 알고 있어야 비로소 올바른 비교가 가능해집니다. 이러한 올바른 비교를 바탕으로, 소비자들은 자신의 소비를 바꿀 것인지에 대한 주체적인 의사결정이 가능해집니다. 따라서 용량과 원재료 구성의 변동 추이를 보여주는 기능을 제공함으로써, 그 전에 몇 번의 슈링크/스킴 플레이션이 언제 있었으며 그때마다 얼마만큼의 변동이 있었는지 소비자들이 확인할 수 있게끔 하였습니다. <br>3) 슈링크/스킴 플레이션으로 인해 더 이상 구매 기준을 충족하지 못하게 된 상품의 소비를 중지하고, 이를 대체할 제품을 찾으려고 할 때, 챗봇 서비스를 이용할 수 있습니다. 사용자가 기존에 소비하던 상품과 가격 혹은 원재료 구성에서 가장 유사한 대체품을 챗봇이 추천해줌으로써 소비자의 고민을 덜어줍니다. |
| (8-4) 과제: 과제의 주요 기능 | 1) 상품 별 용량 및 원재료 변동 추이 대시보드<br>2) 슈링크플레이션 / 스킴플레이션 알림 <br>3) 식료품 구매 판단 도우미 챗봇  |
| (8-5) 과제: 구현 방법 | 1. 변동사항 알림 기능<br> [FE]: 리액트 쿼리의 캐싱 기능을 이용해 데이터를 페칭해온다.<br> [BE]: FCM을 통해 알림을 구현한다.<br> 2. 스킴/슈링크플레이션 정보 시각화 대시보드<br> [FE]: nivo 차트 라이브러리를 이용해 차트를 구현한다.<br> [BE]: Python selenium 및 beautiful soup을 사용하여 정보를 크롤링하고 DB에 저장한다.<br> [ML]: Yolov5 및 EasyOCR 모델을 finetuning하여 상품 정보를 자동으로 DB화하는 파이프라인을 형성한다.<br> 3. 구매 판단 도우미 챗봇<br> [FE]: 라이브러리를 이용하지 않고, useEffect 를 이용해 챗봇의 자동 응답을 비동기적으로 처리한다.<br> [BE]: API 서버와 모델 서버 간의 통신을 통해 사용자가 원하는 기능의 종류를 파악하고, usecase로 관리된 service 함수에 따라 기능을 수행한다.<br> [ML]: KoBERT 모델을 finetuning하여 사용자의 채팅을 서비스 기능 중 하나로 분류한다. |
...
| (8-7) 과제: 기대 효과 및 의의 | 기업의 슈링크/스킴플레이션이 빈번하게 발생하는 상황에서, 소비자들은 Truth Tag의 대시보드 기능을 통해 상품 정보를 정확히 파악할 수 있고, 간편한 검색, 구매 판단 도우미 챗봇을 이용해 간단하게 구매 판단을 내릴 수 있습니다. Truth Tag 를 이용해 간단하게 현명한 소비를 할 수 있다면, 소비자들 사이에 현명한 소비 문화가 정착되는 것을 촉진시킬 수 있고, 최종적으로는 소비자의 식료품 물가 관심을 제고시킴으로써 기업의 무분별한 슈링크/스킴플레이션을 방지할 수 있습니다.  |
