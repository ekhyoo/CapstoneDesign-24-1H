...
| (8-3) 과제: 제안 내용 | 1. 사용자가 등록한 일정과 소비내역 데이터를 바탕으로 소비 패턴을 분석하고, 각 일정에 대한 예산 사용량을 예측하고 제공한다. 또한 사용자의 니즈에 맞는 소비 가이드라인을 제시하여 한 달 예산을 적절하게 분배하여 사용하도록 돕는다. <br>2. 건강한 소비 습관을 보다 쉽게 구축하도록 가계부에 일상적으로 사용하는 일정 리스트 기능을 넣고, ocr 기술을 적용 소비 기록의 편의성을 높인다. 또한 사용량 공유, 챌린지로  친구와 서비스 사용을 함께함으로써 돈 관리에 적당한 동기부여와 재미를 제공한다. |
